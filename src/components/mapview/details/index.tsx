...
import ReactMapGL, { FullscreenControl, MapRef, Marker, NavigationControl } from 'react-map-gl';
...
const Details = (feature: Feature) => {
    ...
    const [longitude, setLongitude] = useState<number | undefined>(geo?.coordinates[0]);
    const [latitude, setLatitude] = useState<number | undefined>(geo?.coordinates[1]);
    ...
    return (
        <div style={{ width: '600px', height: '600px' }}>
            ...
            >
                ...
                >
                    {view === 'airports' ? <div>{airportIcon}</div> : <div>{stadiumIcon}</div>}
                </Marker>
            </ReactMapGL>
        </div>
    );
